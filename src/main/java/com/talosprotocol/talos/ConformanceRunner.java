...
public class ConformanceRunner implements CommandLineRunner {
    ...
    private Handler getHandler(JsonNode vec) {
        ...
        if (testId.startsWith("signing_") || testId.startsWith("sign_") || testId.startsWith("verify_")) return new SigningHandler();
        if (testId.startsWith("canonical_")) return new CanonicalHandler();
        if (testId.startsWith("kdf_") || testId.startsWith("header_")) return new MicroVectorHandler();
        return null;
    }
    ...
    class MicroVectorHandler implements Handler {
        public void run(JsonNode vec) throws Exception {
             ...
             if (testId.equals("kdf_rk_root_ratchet")) {
                 ...
                 if (!nextRk.equals(expected.get("new_rk").asText())) throw new RuntimeException("RK mismatch");
                 if (!nextCk.equals(expected.get("new_ck").asText())) throw new RuntimeException("CK mismatch");
                 ...
                 if (!b64u(nextCk).equals(expected.get("next_ck").asText())) throw new RuntimeException("Next CK mismatch");
                 if (!b64u(mk).equals(expected.get("mk").asText())) throw new RuntimeException("MK mismatch");
    ...
    private void runTrace(JsonNode root) throws Exception {
        ...
        byte[] aliceEphPriv = aliceInit.has("ephemeral_private") ? d64u(aliceInit.get("ephemeral_private").asText()) : null;
        ...
        bob.initializeAsResponder(bobIdPriv, bobSPKPriv, null, d64u(aliceInit.get("identity_public").asText()), aliceEphPub);
        ...
        for (JsonNode step : root.get("steps")) {
            ...
            if (action.equals("encrypt")) {
                ...
                if (step.has("wire_message_b64u") && !gotWireb64u.equals(step.get("wire_message_b64u").asText())) {
                    throw new RuntimeException("Trace encryption mismatch at step " + step.get("step") + ". Got " + gotWireb64u);
                }
            } else {
    ...
    private String b64u(byte[] data) { return Base64.getUrlEncoder().withoutPadding().encodeToString(data); }
    private byte[] d64u(String s) { return Base64.getUrlDecoder().decode(s); }
